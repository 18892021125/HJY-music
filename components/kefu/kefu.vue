<template>
	<view>
		<uni-nav-bar left-icon="back" @clickLeft="back" title="客服" backgroundColor="#f8eee2" ></uni-nav-bar>
			...
			<view  class="flex-column-start" v-for="(x,i) in msgList" :key="i">
				...
				<view v-if="!x.my" class="flex-row-start margin-left margin-top one-show" >
					<view class="chat-img flex-row-center">
						<image style="height: 95rpx;width: 95rpx;" src="../../static/img/kefutouxiang.png" mode="aspectFit"></image>
					</view>
	...
	export default {	
		...
		data() {
			return {
				...
				msgList:[{my:false,msg:"你好我是何家营鼓乐娜娜,请问有什么问题可以帮助您?",
				type:1,questionList:["如何购买课程","如何搜索乐谱","如何开启直播"]}],
				msg:"",
				go:0,
				srcollHeight:0
			}
		...
		methods: {
			...
			msgKf(x){				
				...
				setTimeout(()=>{
					// 取消loading
					this.msgLoad=false
					this.msgList.push({my:false,msg:"娜娜还在学习中,没能明白您的问题,您点击下方提交反馈与问题,我们会尽快人工处理",type:0,questionList:["如何购买课程","如何搜索乐谱","如何开启直播"]})
					this.msgList.push({my:false,msg:"请稍后，客服人员即将上线",type:-1})
					this.msgList.push({my:false,msg:"根据您的问题,已为您匹配了下列问题(多个答案模板)",type:2,questionList:["如何购买课程","如何搜索乐谱","如何开启直播"]})
					this.msgGo()
				},2000)
			},
			// 不建议输入框聚焦时操作此动画
